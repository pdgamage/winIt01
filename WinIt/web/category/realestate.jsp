    ...
    <body>
        ...
        <%
            ...
            if (itemlist != null && !itemlist.isEmpty()) {
                boolean hasRealEstate = false; // To check if any antique items are found
                out.println("<div class='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 justify-items-center gap-6'>");
                for (Item i : itemlist) {
                    if ("RealEstate".equalsIgnoreCase(i.getCategory())) { // Check for category "RealEstate"
                        hasRealEstate = true;
        %>
        ...
        <%
                ...
                out.println("</div>");
                if (!hasRealEstate) { // If no antiques are found, show a message
