    ...
    <body>
        ...
        <%
            ...
            if (itemlist != null && !itemlist.isEmpty()) {
                boolean hasVehicles = false; // To check if any antique items are found
                out.println("<div class='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 justify-items-center gap-6'>");
                for (Item i : itemlist) {
                    if ("Vehicles".equalsIgnoreCase(i.getCategory())) { // Check for category "Vehicles"
                        hasVehicles = true;
        %>
        ...
        <%
                ...
                out.println("</div>");
                if (!hasVehicles) { // If no antiques are found, show a message
